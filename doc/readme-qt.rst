WineCoin-qt: Qt4 GUI for WineCoin
===============================
...
Alternatively, install Qt Creator and open the `WineCoin-qt.pro` file.

An executable named `WineCoin-qt` will be built.
...
A warning for people using the *static binary* version of WineCoin on a Linux/UNIX-ish system (tl;dr: **Berkely DB databases are not forward compatible**).

The static binary version of WineCoin is linked against libdb5.3.
...
installed causes WineCoin-qt to crash intermittently.  The issue has been reported as `launchpad bug 857790`_, but
isn't yet fixed.
