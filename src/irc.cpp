...
void ThreadIRCSeed2(void* parg)
{
    ...
    while (!fShutdown)
    {
        ...
        if (fTestNet) {
            Send(hSocket, "JOIN #WineCoinTEST2\r");
            Send(hSocket, "WHO #WineCoinTEST2\r");
        } else {
            // randomly join #WineCoin00-#WineCoin05
            // int channel_number = GetRandInt(5);

            // Channel number is always 0 for initial release
            int channel_number = 0;
            Send(hSocket, strprintf("JOIN #WineCoin%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #WineCoin%02d\r", channel_number).c_str());
        }
